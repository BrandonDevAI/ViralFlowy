...
export default function AppPage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!isLoggedIn) {
        router.replace('/');
      } else if (!hasActiveSubscription && !isAdminAccount && !profile?.is_pro) {
        router.replace('/');
      }
    }
  }, [authLoading, isLoggedIn, hasActiveSubscription, isAdminAccount, profile, router]);
  ...
  const handleGenerateUrl = useCallback(async () => {
    ...
    try {
      const result = (await replicateViralUrl(
        viralUrl,
        brandIdea,
        videoContext || undefined
      )) as CompleteVideoOutput & { originalVideoAnalysis?: OriginalVideoAnalysis };
            ...
            {isLoggedIn && profile ? (
              <div className="flex items-center gap-2">
                <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/[0.04] border border-white/[0.06]">
                  ...
                  <span className="text-[11px] font-medium text-gray-300 max-w-[90px] truncate">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-semibold hover:bg-indigo-500/20 transition-all"
              >
                Iniciar sesión
              </button>
            )}
              ...
              {mode === 'idea' && (
                <div className="p-5">
                  ...
                  <textarea
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)) handleGenerateIdea();
                    }}
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-amber-500/[0.07] border border-amber-500/20">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-amber-300">
                          Función exclusiva — Plan Creador o Pro
                        </p>
                        <p className="text-[11px] text-amber-400/70 mt-0.5">
                          Actualiza tu plan para replicar cualquier video viral.
                        </p>
                      </div>
                      <button
                        type="button"
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-2">
                      ...
                      <span className="text-gray-600 normal-case font-normal">
                        (opcional pero mejora mucho el resultado)
                      </span>
                    </label>
                    <textarea
                      value={videoContext}
                    ...
                    >
                      ...
                      {productImagePreview ? (
                        <>
                          ...
                          <span className="text-xs font-semibold text-amber-300 z-20 drop-shadow-md">
                            Producto listo
                          </span>
                        </>
                      ) : (
                    ...
                    >
                      ...
                      {refImagePreview ? (
                        <>
                          ...
                          <span className="text-xs font-semibold text-orange-300 z-20 drop-shadow-md">
                            Ref. lista
                          </span>
                        </>
                      ) : (
                    ...
                    <label className="flex items-start gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.05] cursor-pointer hover:bg-white/[0.04] transition-colors">
                      ...
                      <div className="flex-1 flex flex-col">
                        ...
                        <span className="text-[11px] text-gray-500 mt-0.5">
                          Actívalo solo cuando estés seguro del texto.
                        </span>
                      </div>
                    </label>
                  </div>
            ...
            <div className="mt-8 space-y-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-[11px] text-gray-600 flex items-center gap-1.5">
                  ...
                  {mode === 'url'
                    ? 'Analizando el video viral y adaptando...'
                    : mode === 'ads'
                    ? 'Diseñando el Flyer y redactando el Copy...'
          ...
          {carouselOutput && !isLoading && (
            <div id="results" className="mt-8">
              ...
              <div className="mt-8 text-center">
                <button
                  onClick={handleReset}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.07] text-gray-400 hover:text-white text-sm font-medium transition-all"
                >
                  <RotateCcw className="w-3.5 h-3.5" />
                  Nuevo carrusel
                </button>
                    ...
                    {adsOutput.flyerUrl ? (
                      ...
                      <div className="text-center p-6 opacity-60">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Activa la opción de DALL-E para crear la imagen publicitaria.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex flex-col gap-4">
                  <div>
                    ...
                    <div className="space-y-2">
                      {adsOutput.headlines.map((headline, idx) => (
                        <div
                          key={idx}
                          className="flex gap-2 p-3 rounded-xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] transition-colors group"
                        >
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {adsOutput.primaryTexts.map((text, idx) => (
                        <div
                          key={idx}
                          className="flex flex-col gap-2 p-3 rounded-xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] transition-colors group"
                        >
                          ...
                          <div className="flex justify-end">
                            <button
                              onClick={() => copyToClipboard(text, `pt-${idx}`)}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 rounded-xl bg-amber-500/[0.05] border border-amber-500/10">
                      <p className="text-[10px] text-amber-500/70 uppercase font-bold mb-1">
                        Mejor Botón CTA
                      </p>
                    ...
                    <div className="p-3 rounded-xl bg-blue-500/[0.05] border border-blue-500/10">
                      <p className="text-[10px] text-blue-500/70 uppercase font-bold mb-1">
                        Audiencia Sugerida
                      </p>
                      <p className="text-xs font-medium text-blue-100 leading-snug">
                        {adsOutput.targetAudience}
                      </p>
                    </div>
                  </div>
                </div>
